  ...
  async (server) => {
    ...
    server.patch(
      PATH,
      {
        schema: {
          ...
          body: z.object({
            ...
            mentions: z.array(z.string().trim().min(1).max(32)).max(10).optional(),
      ...
      async (req, res) => {
        ...
        if (req.body.mentions !== undefined) data.mentions = req.body.mentions;
        ...
        if (req.body.tags !== undefined) {
          // fork: tags are global/shared, so any existing tag id is valid
          const tagCount = await db.$count(tags, inArray(tags.id, req.body.tags));
